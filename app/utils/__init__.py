...
# Validação de email conforme expressões regulares: módulo `re`
...
def response(status_code: int, message: str, target: str = None):
	...
	Args:
		...
		message: str - Mensagem de erro ou sucesso a ser exibida;
		target: str = None;
...
def get_validate(data: any, schema: 'dict[str, type]') -> 'dict[str, any]':
	'''
	Realiza a verificação de campos via `request.get_json()`, 
	comparando se os campos desejados conforme schema são
	os mesmos que estão vindos por request.
...
def do_add_commit(inst: db.Model):
	...
	Args:
		inst: isntância de db.Model - linha do banco de dados.
